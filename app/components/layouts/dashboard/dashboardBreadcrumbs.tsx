...
const useDashboardBreadcrumbs = () => {
    ...
    const breadcrumbs = useMemo(() => {
        return matches.filter((match) => match.handle && (match.handle as BreadcrumbHandle).breadcrumb)
            .map((match) => (match.handle as BreadcrumbHandle).breadcrumb);
    }, [matches]);
    return breadcrumbs;
}

const BreadcrumbObj = ({ title, path, hasTrailingSlash }: { title: string, path: string, hasTrailingSlash: boolean }) => {
